...
function cambiarTabAdmin(tab) {
    ...
    if (btnUsuarios)
        btnUsuarios.className = inactiveClass;
    if (btnRoles)
        btnRoles.className = inactiveClass;
    if (tab === "import") {
        if (secImport)
        ...
        if (btnRoles)
            btnRoles.className = activeClass;
        ...
        if (btnUsuarios)
            btnUsuarios.className = activeClass;
        cargarUsuariosSupabase();
    }
}
...
function parseFechaISO(str) {
    if (!str || typeof str !== 'string' || !str.trim() || str.trim().toUpperCase() === 'N/A' || str.trim().toUpperCase() === 'PENDIENTE')
        return null;
...
async function procesarImportacionMasiva() {
    ...
    try {
        ...
        if (extension === "xlsx" || extension === "xls") {
            ...
            if (rows.length <= 1)
                throw new Error("El archivo CSV no contiene registros suficientes.");
    ...
    if (tablaTarget === "maestro_activos") {
        ...
        return rawJson.map(row => {
            ...
            if (Array.isArray(rawTareas)) {
                ...
                try {
                    tareasParsed = JSON.parse(rawTareas);
                }
                catch (e) {
                    tareasParsed = [];
                }
