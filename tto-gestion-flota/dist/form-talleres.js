...
async function cargarUnidadesFlotaModal() {
    ...
    tbody.innerHTML = `
        <tr>
            <td colspan="6" class="p-4 text-center text-cyan-400 font-bold uppercase text-[10px]">
...
function seleccionarUnidadFlota(idUnidad, marca, tipoFlota, vin, modelo, color, anio, tipoVehiculo, gerencia, responsable) {
    ...
    if (inputUnidad)
        inputUnidad.value = idUnidad;
    if (inputMarca)
        inputMarca.value = marca;
    if (selectFlota)
        selectFlota.value = tipoFlota;
    if (inputVin)
        inputVin.value = vin || "";
    if (inputModelo)
    ...
    if (inputTipoVeh)
        inputTipoVeh.value = tipoVehiculo || "";
    ...
    if (inputChofer)
        inputChofer.value = responsable || '';
    cerrarModalSeleccionarUnidad();
    if (window.SIAGOP_UI) {
        SIAGOP_UI.success("Unidad Seleccionada", `Se cargó la unidad ${idUnidad} (${marca}) al formulario.`);
    }
}
const CLAVE_COLA_OFFLINE = "SIAGOP_COLA_PETICIONES_OFFLINE";
const CLAVE_RESPALDO_MATRIZ = "SIAGOP_RESPALDO_LOCAL_MATRIZ";
...
async function procesarColaOffline() {
    ...
    if (cola.length === 0)
        return;
    if (window.SIAGOP_UI) {
        SIAGOP_UI.info("Sincronizando...", `Enviando ${cola.length} operación(es) guardada(s) sin conexión.`);
    }
...
async function confirmarIdentidad(event) {
    ...
    if (!selectOperador || !inputPassword)
        return;
...
async function cargarTablaEditable() {
    ...
    if (window.SIAGOP_UI_UTILS && typeof window.SIAGOP_UI_UTILS.renderizarSkeletonTabla === 'function') {
        ...
        tbody.innerHTML = `
            <tr class="block md:table-row">
                <td colspan="7" class="block md:table-cell p-8 text-center text-cyan-400 font-bold uppercase tracking-widest text-[10px]">
    ...
    try {
        ...
        listaRegistrosPanel = filasCrudas.map(u => {
            ...
            for (let key in u) {
                ...
                if (typeof val === 'string' && (val.includes('drive.google.com') || val.includes('docs.google.com'))) {
                    let driveId = '';
                    if (val.includes('id='))
                        driveId = val.split('id=')[1]?.split('&')[0];
                    else if (val.includes('/file/d/'))
                        driveId = val.split('/file/d/')[1]?.split('/')[0];
                    if (driveId)
                        val = `https://drive.google.com/thumbnail?id=${driveId}&sz=w1200`;
                }
        ...
        if (!respaldoLocal || respaldoLocal.length === 0) {
            ...
            if (localStr)
                respaldoLocal = JSON.parse(localStr);
        }
        if (respaldoLocal && respaldoLocal.length > 0) {
            listaRegistrosPanel = respaldoLocal;
            renderizarMatriz(listaRegistrosPanel);
            if (window.SIAGOP_UI) {
                SIAGOP_UI.warning("Modo Offline Activo", "Mostrando datos guardados localmente en caché.");
            }
        }
        else {
...
function renderizarMatriz(datos) {
    ...
    const htmlFilas = [...datos].reverse().map(reg => {
        ...
        if (reg.Estatus === "En Proceso") {
            badgeEstatus = `<span class="bg-cyan-500/10 border border-cyan-500/30 text-cyan-600 dark:text-cyan-400 px-2.5 py-1 rounded-lg text-[9px] font-black tracking-widest uppercase">⚙️ En Proceso</span>`;
...
function previsualizarImagen(input, idContenedor) {
    if (window.SIAGOP_UI_UTILS && typeof window.SIAGOP_UI_UTILS.previsualizarImagen === 'function') {
...
function limpiarPrevia(idInput, idContenedor) {
    if (window.SIAGOP_UI_UTILS && typeof window.SIAGOP_UI_UTILS.limpiarPrevia === 'function') {
...
async function guardarNuevoRegistro(event) {
    ...
    if (fileInput.files.length > 0) {
        ...
        if (!valRes.valido) {
            SIAGOP_UI.error("Archivo adjunto no válido", valRes.mensaje);
            return;
        }
    }
    btn.disabled = true;
    ...
    if (!navigator.onLine) {
        ...
        cerrarModalNuevo();
        SIAGOP_UI.warning("Sin Conexión", "El registro se guardó en la cola offline.");
        btn.disabled = false;
    ...
    try {
        ...
        if (res.status === "SUCCESS") {
            cerrarModalNuevo();
            await cargarTablaEditable();
            SIAGOP_UI.success("Registro Exitoso", "La unidad ha sido ingresada correctamente a la base de datos central.");
        }
        else {
        ...
        encolarPeticionOffline(payload);
        cerrarModalNuevo();
        SIAGOP_UI.warning("Modo Offline Activado", "Error de comunicación. La solicitud fue encolada.");
    }
    finally {
        btn.disabled = false;
        btn.innerHTML = `<i class="fa-solid fa-square-check"></i> Registrar Ingreso`;
...
function actualizarInterfazAvanceModal(porcentaje) {
    ...
    if (lblAvance)
        lblAvance.textContent = porcentaje + "%";
...
async function guardarEdicionModal(event) {
    ...
    if (!navigator.onLine) {
        encolarPeticionOffline(payload);
        cerrarModalEditar();
        SIAGOP_UI.warning("Sin Conexión", "La edición se guardó localmente.");
        btn.disabled = false;
    ...
    try {
        ...
        if (res.status === "SUCCESS") {
            cerrarModalEditar();
            await cargarTablaEditable();
            SIAGOP_UI.success("Actualización Correcta", "Los cambios en el diagnóstico han sido sincronizados.");
        }
        else {
...
async function confirmarEliminarRegistro() {
    ...
    const confirmacion = await SIAGOP_UI.confirm("¿Eliminar Registro?", `Esta acción borrará la unidad ${escapeHTML(unidad)} (ID #${escapeHTML(id)}) de la base de datos y sus fotos en Drive.`, "Eliminar", "Cancelar");
    if (!confirmacion)
        return;
    const confirmacionFinal = await SIAGOP_UI.confirm("Confirmación Final", "¿Está absolutamente seguro? Esta operación no se puede deshacer.", "SÍ, ELIMINAR", "VOLVER");
    ...
    if (!navigator.onLine) {
        ...
        SIAGOP_UI.warning("Eliminación Encolada", "Se procesará el borrado al reconectarse.");
        return;
    }
    try {
        const response = await fetch(APP_CONFIG.URL_API, {
        ...
        if (res.status === "SUCCESS") {
            ...
            await cargarTablaEditable();
            SIAGOP_UI.success("Registro Eliminado", "La unidad y sus archivos asociados han sido removidos.");
        }
        else {
