...
(function () {
    ...
    async function prepareRecordAssets(client, bucketName, record, id) {
        ...
        try {
            ...
            if (window.SIAGOP_SIGN_UPLOAD_API_KEY)
                signHeaders['x-api-key'] = window.SIAGOP_SIGN_UPLOAD_API_KEY;
            if (window.SIAGOP_SIGN_UPLOAD_JWT)
                signHeaders['Authorization'] = 'Bearer ' + window.SIAGOP_SIGN_UPLOAD_JWT;
            ...
            if (typeof out.tareas === 'string') {
                try {
                    out.tareas = JSON.parse(out.tareas);
                }
                catch (e) { /* keep string */ }
            }
            return out;
    ...
    async function syncAndUpsert(tableName, rows, opts = {}) {
        ...
        for (const r of rows) {
            ...
            if (ready.flota && !ready.tipo_flota)
                ready.tipo_flota = ready.flota;
            if (ready.nombre_taller_ext && !ready.taller_ext)
                ready.taller_ext = ready.nombre_taller_ext;
            if (ready.unidad && !ready.id_unidad)
                ready.id_unidad = ready.unidad;
            if (ready.fecha_ingreso)
                ready.fecha_ingreso = parseCustomDateToISO(ready.fecha_ingreso);
            if (ready.fecha_salida)
                ready.fecha_salida = parseCustomDateToISO(ready.fecha_salida);
            if (ready.ubicacion_taller_fecha)
                ready.ubicacion_taller_fecha = parseCustomDateToISO(ready.ubicacion_taller_fecha);
