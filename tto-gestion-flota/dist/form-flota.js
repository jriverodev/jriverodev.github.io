...
function actualizarPaginacionUI(totalItems) {
    ...
    if (paginaActualFlota > totalPaginas)
        paginaActualFlota = totalPaginas;
    ...
    if (infoEl)
        infoEl.textContent = `Mostrando ${inicio} - ${fin} de ${totalItems} activos`;
    const labelEl = document.getElementById("labelPaginaActual");
    if (labelEl)
        labelEl.textContent = `${paginaActualFlota} / ${totalPaginas}`;
    const btnAnt = document.getElementById("btnPaginaAnterior");
    if (btnAnt)
        btnAnt.disabled = paginaActualFlota === 1;
    const btnSig = document.getElementById("btnPaginaSiguiente");
    if (btnSig)
        btnSig.disabled = paginaActualFlota === totalPaginas;
}
...
async function confirmarEliminarRegistro() {
    const idUnidad = document.getElementById("edit-id-unidad").value;
    const confirmacion = await SIAGOP_UI.confirm("¿Eliminar Activo?", `Esta acción borrará definitivamente la unidad ${escapeHTML(idUnidad)} y sus documentos asociados del Maestro de Activos.`, "Eliminar", "Cancelar");
    if (!confirmacion)
        return;
    const confirmacionFinal = await SIAGOP_UI.confirm("Confirmación Final", "¿Está seguro de querer remover este activo de la flota de manera permanente?", "SÍ, ELIMINAR", "VOLVER");
