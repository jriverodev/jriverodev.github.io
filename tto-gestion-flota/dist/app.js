...
async function handleLocalApiGateway(payload) {
    ...
    if (accion === 'validar_token') {
        ...
        if (storedRol !== 'admin' && storedMod && storedMod !== 'TODOS' && moduloReq) {
            if (moduloReq === 'TALLERES' && storedMod !== 'TALLERES' && storedRol !== 'operador_talleres')
                esPermitido = false;
            if (moduloReq === 'FLOTA' && storedMod !== 'FLOTA' && storedRol !== 'operador_flota')
                esPermitido = false;
        }
        return toJsonResponse({
            status: 'SUCCESS',
    ...
    if (accion === 'crear' || accion === 'editar') {
        ...
        if (navigator.onLine && client) {
            try {
                ...
                if (payloadRemoto.flota && !payloadRemoto.tipo_flota)
                    payloadRemoto.tipo_flota = payloadRemoto.flota;
                if (payloadRemoto.nombre_taller_ext && !payloadRemoto.taller_ext)
                    payloadRemoto.taller_ext = payloadRemoto.nombre_taller_ext;
                if (payloadRemoto.unidad && !payloadRemoto.id_unidad)
                    payloadRemoto.id_unidad = payloadRemoto.unidad;
                // Type conversions
                ...
                if (typeof payloadRemoto.tareas === 'string') {
                    try {
                        payloadRemoto.tareas = JSON.parse(payloadRemoto.tareas);
                    }
                    catch (eJson) { }
                }
            ...
            try {
                ...
                if (payloadRemoto.documento_eliminar) {
                    ...
                    if (client && client.storage) {
                        ...
                        const posiblesRutas = [];
                        if (pathEliminar)
                            posiblesRutas.push(pathEliminar);
...
if (originalFetch) {
    window.fetch = async function (input, init = {}) {
        ...
        const isApiRequest = Boolean(url && (url === APP_CONFIG.URL_API ||
...
async function obtenerUrlFirmadaStorage(urlOrPath, expiresIn = 7200, bucketDefault = 'siagop-archivos') {
    ...
    if (!path)
        return clean; // Retain Base64 or Google Drive thumbnail links unchanged
        ...
        try {
            ...
            if (!error && Array.isArray(data) && data.length > 0) {
                ...
                const filtrados = data.filter(u => {
                    ...
                    if (mod === 'TODOS' || rol === 'admin')
                        return true;
                    if (moduloRequerido.toUpperCase() === 'TALLERES' && (mod === 'TALLERES' || rol === 'operador_talleres'))
                        return true;
                    if (moduloRequerido.toUpperCase() === 'FLOTA' && (mod === 'FLOTA' || rol === 'operador_flota'))
                        return true;
                    return false;
                });
...
function parseCustomDateToISO(str) {
    if (!str || typeof str !== 'string' || !str.trim() || str.trim().toUpperCase() === 'N/A' || str.trim().toUpperCase() === 'PENDIENTE' || str.trim().toUpperCase() === 'S/F')
        return null;
...
async function procesarSincronizacionPendiente(key = SYNC_QUEUE_KEY) {
    ...
    if (!APP_CONFIG || !APP_CONFIG.URL_API)
        return;
    let queue = JSON.parse(localStorage.getItem(key) || '[]');
    if (queue.length === 0)
        return;
