...
const SIAGOP_CRYPTO = (function () {
    ...
    async function derivarClaveCrypto(passwordSecret, saltBuffer) {
        const keyMaterial = await window.crypto.subtle.importKey('raw', stringToBuffer(passwordSecret), { name: 'PBKDF2' }, false, ['deriveKey']);
    ...
    async function cifrarDatos(datos) {
        try {
            ...
            const ciphertextBuffer = await window.crypto.subtle.encrypt({ name: 'AES-GCM', iv: iv }, cryptoKey, stringToBuffer(jsonStr));
            return {
    ...
    async function descifrarDatos(paqueteCifrado) {
        try {
            ...
            const decryptedBuffer = await window.crypto.subtle.decrypt({ name: 'AES-GCM', iv: ivBuffer }, cryptoKey, ciphertextBuffer);
