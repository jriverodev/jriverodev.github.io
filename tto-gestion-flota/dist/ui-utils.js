...
(function () {
    function previsualizarImagenImpl(input, idContenedor) {
        ...
        if (typeof input === 'string') {
            const url = typeof window.normalizarUrlStorage === 'function' ? window.normalizarUrlStorage(input) : input;
        ...
        if (input && input.files && input.files[0]) {
            const valRes = typeof window.validarArchivoAdjunto === 'function' ? window.validarArchivoAdjunto(input.files[0]) : { valido: true };
            if (!valRes.valido) {
    ...
    function findPreviewPair() {
        ...
        for (const p of pairs) {
            if (document.getElementById(p.container) || document.getElementById(p.input))
                return p;
        }
        return null;
    }
    function isDevMode() {
        ...
        if (!host)
            return true; // file:// may have empty hostname
        return host === 'localhost' || host === '127.0.0.1' || host === '';
    }
    function addDebugFooterButtons() {
        if (!(isDevMode() || window.SIAGOP_DEBUG_UI === true))
            return;
