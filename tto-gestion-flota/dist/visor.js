...
function tiempoTranscurrido(fechaInput) {
    ...
    if (isNaN(fecha.getTime()))
        return escapeHTML(String(fechaInput));
...
async function cargarDatosAnaliticos() {
    ...
    try {
        ...
        if (res.status !== "SUCCESS") {
            if (tbody)
                tbody.innerHTML = `<tr class="block md:table-row"><td colspan="9" class="block md:table-cell p-6 text-center text-red-500 uppercase tracking-widest text-[10px] font-bold">Error: ${escapeHTML(res.message)}</td></tr>`;
            return;
        }
        ...
        if (tbody)
            tbody.innerHTML = `<tr class="block md:table-row"><td colspan="9" class="block md:table-cell p-6 text-center text-red-500 uppercase font-bold text-[10px]">Error fatal conectando con la red central.</td></tr>`;
    }
}
...
function filtrarVisor() {
    ...
    const filtrados = datosUnidadesGlobal.filter(reg => {
        ...
        if ((fDesdeTs !== null || fHastaTs !== null) && reg.Fecha_Registro_Timestamp) {
            if (fDesdeTs !== null && reg.Fecha_Registro_Timestamp < fDesdeTs)
                matchesFecha = false;
            if (fHastaTs !== null && reg.Fecha_Registro_Timestamp > fHastaTs)
                matchesFecha = false;
        }
...
function renderizarVisor(datos) {
    ...
    const htmlFilas = [...datos].reverse().map(reg => {
        if (reg.Estatus === "Por Atender")
            porAtender++;
        else if (reg.Estatus === "En Proceso")
            enProceso++;
        else
            listos++;
...
async function abrirModalDetalle(id) {
    ...
    if (typeof obtenerUrlFirmadaStorage === 'function') {
        if (reg.Foto_Antes)
            reg.Foto_Antes = await obtenerUrlFirmadaStorage(reg.Foto_Antes);
        if (reg.Foto_Despues)
            reg.Foto_Despues = await obtenerUrlFirmadaStorage(reg.Foto_Despues);
    }
...
function renderizarGraficos(talleresData, espera, proceso, listos) {
    ...
    if (!canvasTalleres || !canvasEstatus)
        return;
...
function actualizarGraficosVivos() {
    if (instanciaChartTalleres)
        instanciaChartTalleres.resize();
    if (instanciaChartEstatus)
        instanciaChartEstatus.resize();
}
...
function exportarAPDF() {
    ...
    if (datosUnidadesGlobal.length === 0)
        return SIAGOP_UI.error("Error de Exportación", "No hay datos para exportar a PDF.");
    html2pdf().set({
        margin: 0.3,
