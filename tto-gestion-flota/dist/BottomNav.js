...
(function () {
    ...
    function renderizarTabs(container) {
        ...
        const tabsFiltrados = RUTAS_TAB.filter(tab => {
            if (tab.hash === '#/activos' && !perm.esFlota)
                return false;
            if (tab.hash === '#/mantenimiento' && !perm.esTalleres)
                return false;
            if (tab.hash === '#/roles' && !perm.esAdmin)
                return false;
            return true;
        });
        container.innerHTML = tabsFiltrados.map(tab => {
            ...
            let esActivo = false;
            if (tab.hash === '#/dashboard' && (currentPath === 'panel.html' || currentPath === 'visor.html' || currentPath === 'patio.html'))
                esActivo = true;
            else if (tab.hash === '#/activos' && (currentPath === 'form-flota.html' || currentPath === 'visor-flota.html'))
                esActivo = true;
            else if (tab.hash === '#/mantenimiento' && (currentPath === 'form-talleres.html' || currentPath === 'visor-talleres.html'))
                esActivo = true;
            else if (tab.hash === '#/roles' && currentPath === 'admin.html' && currentHash.includes('roles'))
                esActivo = true;
