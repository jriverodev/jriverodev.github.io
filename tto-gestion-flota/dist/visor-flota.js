...
async function cargarDatosAnaliticos() {
    ...
    try {
        ...
        if (res.status !== "SUCCESS") {
            if (tbody)
                tbody.innerHTML = `<tr class="block md:table-row"><td colspan="7" class="block md:table-cell p-6 text-center text-red-500 uppercase tracking-widest text-[10px] font-bold">Error: ${escapeHTML(res.message)}</td></tr>`;
            return;
        }
        ...
        if (tbody)
            tbody.innerHTML = `<tr class="block md:table-row"><td colspan="7" class="block md:table-cell p-6 text-center text-red-500 uppercase font-bold text-[10px]">Error fatal conectando con la red central.</td></tr>`;
    }
}
...
function actualizarPaginacionUI(totalItems) {
    ...
    if (labelEl)
        labelEl.textContent = `${paginaActual} / ${totalPaginas}`;
...
function renderizarGraficos(liviana, pesada, marcasData) {
    ...
    if (!canvasFlota || !canvasMarcas)
        return;
    if (instanciaChartFlota)
        instanciaChartFlota.destroy();
    if (instanciaChartMarcas)
        instanciaChartMarcas.destroy();
...
function actualizarGraficosVivos() {
    if (instanciaChartFlota)
        instanciaChartFlota.resize();
    if (instanciaChartMarcas)
        instanciaChartMarcas.resize();
}
async function exportarAExcel() {
    if (datosActivosGlobal.length === 0)
        return SIAGOP_UI.error("Error", "No hay datos para exportar.");
